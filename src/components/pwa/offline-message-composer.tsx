...
import { Clock, RotateCcw, Send, WifiOff } from 'lucide-react';
import { useCallback, useEffect, useRef, useState } from 'react';

import { Button } from '@/components/ui/button';
import { Textarea } from '@/components/ui/textarea';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';
import { useBackgroundSync, useOfflineStatus } from '@/hooks/use-pwa';
...
export function OfflineMessageComposer({
  ...
  const sendPendingMessages = useCallback(async () => {
    ...
    for (const message of messagesToSend) {
      try {
        ...
        // Will be re-queued via failedMessages
  ...
  }, [pendingMessages, onSend]);

  useEffect(() => {
    if (wasOffline && !isOffline && pendingMessages.length > 0) {
      void sendPendingMessages();
    }
  }, [wasOffline, isOffline, pendingMessages, sendPendingMessages]);
  ...
  const handleSend = async () => {
    ...
    if (isOffline) {
      ...
      try {
        ...
        // If send fails, queue for retry automatically
        queueMessage(messageContent);
      } finally {
        setIsSending(false);
      }
  ...
  const handleRetry = async (messageId: string) => {
    ...
    try {
      ...
      // Re-queue if failed — automatically restored to pending
      setPendingMessages((prev) => [...prev, message]);
    } finally {
      setIsSending(false);
    }
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300">
                {isOffline ? "Will send when you're back online" : 'Sending now...'}
              </p>
            </div>
            {isOffline && <WifiOff className="h-4 w-4 text-amber-600 dark:text-amber-400" />}
          </div>
